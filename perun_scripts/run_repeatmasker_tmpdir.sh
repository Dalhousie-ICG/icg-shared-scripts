...
# this version of the script will copy the input files to the local disk of
# the node that does the actual computing. It increases the "storage throughput"
# and may help speed up RepeatModeler

# first, copy the input files to the local disk of the compute node
ASSEMBLY="ergo_cyp_genome.fasta"
cp $ASSEMBLY $TMPDIR
# and record the current working directory so we can transfer the output back to it later
WORKDIR=`pwd`

# then, cd to TMPDIR
cd $TMPDIR

# and execute the job (code below)
...
# run repeatmodeler
JOBS=6
RepeatModeler -pa $JOBS -database $NAME -LTRStruct &> repeatmodeler.out
...
# Finally, copy all output files back to the working directory
cp $NAME* repeatmodeler.out $WORKDIR
cp -r $OUTDIR $WORKDIR
